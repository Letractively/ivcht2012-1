...
<form action="editauto" accept="" method="post">
    ...
    <table class="bordered">
            ...
            <td>
                <select name="id_brand" id="Select1" size="1" required>
                    ...
                    <c:forEach items="${brand}" var="brand">
                        <option value="<c:out value="${brand.id}"></c:out>"
                                <c:if test="${brand.id==auto.id_brand}">SELECTED="" </c:if>><c:out
            ...
            <td><select name="id_types" id="Select2" size="1" required>
                <c:if test="${not empty types}">
                    <c:forEach items="${types}" var="types">
                        <option value="<c:out value="${types.id}"></c:out>"
                                <c:if test="${types.id==auto.id_types}">SELECTED="" </c:if>><c:out
        ...
        <tr>
            ...
            <td><input type="text" name="height" value="<c:out value="${auto.height}"></c:out>"/>x<input type="text"
                                                                                                         name="width"
                                                                                                         value="<c:out value="${auto.width}"></c:out>"/>x<input
                    type="text" name="len" value="<c:out value="${auto.len}"></c:out>"/></td>
        </tr>
        <tr>
            <td>Рейтинг</td>
